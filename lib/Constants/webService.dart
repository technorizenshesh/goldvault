...
class Webservices {
  ...
  static Future<Map<String, dynamic>> postData({required String apiUrl,
    ...
    try {
     // log('the request for $apiUrl is $body');
      response = await http.post(Uri.parse(apiUrl), body: body, headers: {"Authorization":"${userToken==null?"":userToken}"});
  ...
  static Future<Map<String, dynamic>> getMap(String url,
    ...
    try {
      log('the request for url $url is $tempRequest $userToken');
      late http.Response response;
      if (request == null) {
        response = await http.get(Uri.parse(url),headers: {"Authorization":"${userToken==null?"":userToken}"});
      } else {
